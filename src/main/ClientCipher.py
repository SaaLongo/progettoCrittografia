...
def stringToNumbers(string):
    ...
    for character in string:
        number = ord(character) - deltaClient
...
"""
def encryptClient(plaintext, key, gammaValue):
    betaValue = power(g, key, q)
    en_msg, p = encrypt(plaintext, betaValue, gammaValue, key)

    return en_msg,p

def decryptClient(en_msg, p, gammaValue, key):
...
def main():
    ...
    en_msg, p = encryptClient(msg, key,gammaValue)

    dr_msg = decryptClient(en_msg, p, gammaValue, key)
    dmsg = ''.join(dr_msg)
