...
"""
def encryptSV(plaintext, key, gammaValue):
    betaValue = power(g, key, q)
    en_msg, p = encrypt(plaintext, betaValue, gammaValue, key)

    return en_msg,p

def decryptSV(en_msg, p, gammaValue, key):
...
def main():
    ...
    en_msg, p = encryptSV(msg, key,gammaValue)

    dr_msg = decryptSV(en_msg, p, gammaValue, key)
    dmsg = ''.join(dr_msg)
